                    ...
                    for line in file:
                        ...
                        if is_desktop:
                            if line.startswith('Name=') and not app['name']:
                                ...
                                app['icon'] = line[5:]
                            elif line.startswith('Path=') and not app['path']:
                                app['path'] = line[5:]
                            elif line.startswith('NoDisplay=true') or line.startswith('NoDisplay=1'):
                                no_display = True

                    if app['name'] and app['exec'] and not no_display:
                        # Wrap exec so it cd's into Path= first, if present
                        if app['path']:
                            app['exec'] = f"bash -c 'cd \"{app['path']}\" && {app['exec']}'"
                        apps[app['name']] = app
            except Exception:
                pass
